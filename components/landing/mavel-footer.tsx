    ...
    <footer id="contato" className="scroll-mt-24 border-t border-white/10 bg-zinc-950">
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 gap-10 md:grid-cols-4">
          <div className="col-span-2">
            <Link href="/" className="flex items-center gap-2 font-semibold">
              ...
              <span className="text-lg tracking-tight text-white">{settings.storeName.toUpperCase()}</span>
            </Link>
        ...
        <div className="mt-14 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-8 text-xs text-zinc-500 sm:flex-row">
          <p>
            © {new Date().getFullYear()} {settings.storeName.toUpperCase()}. {settings.copyrightText}
          </p>
          <div className="flex items-center gap-4">
            <p>Impressão 3D premium para quem cria sem limites.</p>
            <Link href="/admin/login" className="text-zinc-600 transition-colors hover:text-zinc-400">
              Área Administrativa
            </Link>
          </div>
        </div>
